...
const LocalTaxGrid: React.FC<LocalTaxGridProps> = ({
  ...
  const [preselectedTaxes, setPreselectedTaxes] = useState<LocalTax[]>([]); // Preselected items
  ...
  // Track preselected taxes based on nexusList
  useEffect(() => {
    if (nexusList && Array.isArray(nexusList)) {
      const matchingTaxes = localTaxesArray.filter((tax) =>
        nexusList.some((nexus: any) => nexus.juris_name === tax.juris_name)
      );
      setPreselectedTaxes(matchingTaxes);
    }
  }, [nexusList, localTaxesArray]);

  const formik = useFormik({
    initialValues: {
      selectedTaxes: [] as LocalTax[], // Only user-selected taxes
    },
    validationSchema: Yup.object({
      selectedTaxes: Yup.array().min(1, 'Select at least one tax'),
    }),
    onSubmit: (values: any) => {
      // Combine both preselected and user-selected taxes in the submission data
  ...
  const handleSearchChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSelectAll(false); // Reset select all when filtering
  };
  ...
  // Combine preselected and user-selected taxes for the count display
  const totalSelectedCount =
    preselectedTaxes.length + formik.values.selectedTaxes.length;
  ...
  return (
    <Form onSubmit={formik.handleSubmit}>
      ...
      <div className="d-flex justify-content-between align-items-center mb-3">
        {/* Show total selected including both preselected and user-selected taxes */}
        <p>
          {totalSelectedCount} of {filteredTaxes.length} taxes selected
        </p>
            ...
            <tr key={tax.id}>
              <td className="d-flex align-items-center">
                <Form.Check
                  type="checkbox"
                  checked={
                    preselectedTaxes.some(
                      (preselectedTax) => preselectedTax.id === tax.id
                    ) ||
                    formik.values.selectedTaxes.some(
                      (selectedTax: LocalTax) => selectedTax.id === tax.id
                    )
                  }
                  onChange={(e) => handleTaxChange(tax, e.target.checked)}
                  style={{ marginRight: '10px' }}
                  disabled={preselectedTaxes.some(
                    (preselectedTax) => preselectedTax.id === tax.id
                  )} // Disable preselected
                />
                {tax.juris_name}
