...
import AddLocalTax from './AddLocalTaxes';
...
interface CommonStateUIProps {
  ...
  onRemove: (stateName: string) => void;
  onStateDataChange: (
    stateData: StateData,
...
const StateUI: React.FC<CommonStateUIProps> = ({
  ...
  useEffect(() => {
    // Set default selection for radio buttons based on uiType
    if (uiType === 'typeA') {
      if (stateName === 'Texas') {
        setSelectedTaxType('Single local use tax on all sales');
      } else {
        setSelectedTaxType('simplified sellers use tax');
      }
    } else if (uiType === 'typeB') {
      setSelectedTaxType('as a remote seller');
    } else if (uiType === 'typeE') {
      setSelectedTaxType('sales tax on all sales');
    }
  }, [uiType]);
  ...
  return (
    ...
    >
      ...
      <Card.Body>
        ...
        <Button variant="outline-danger" onClick={() => onRemove(stateName)}>
          Remove
        </Button>
        <Modal
